...
// message (C, right). Below them Primary/Secondary news leads: Primary fills
// the left column (D/F/H), Secondary the right (E/G/I); then the full-width
// sections follow in order - Whole School (W), Foundation (V), Sixth Form (X)
// and Co-Curricular (Y).
const SLOT_LABELS = {
  A: 'A - Header & quote of the week (fixed)',
  B: 'B - Upcoming events · left column (fixed)',
  C: "C - Principal's message · right column (fixed)",
  W: 'W - Whole School · full width, under the Primary/Secondary columns',
...
function columnRule(section) {
  if (section === 'whole_school') return 'Whole School stories live in their own full-width section (W) under the Primary/Secondary columns.';
